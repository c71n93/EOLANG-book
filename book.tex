...
The bytes data type object represents an abstraction of an sequence of bytes and can be empty or contain an theoretically unlimited number of bytes.
Fully Qualified Name: org.eolang.bytes (no aliasing or FQN reference required since the object is automatically imported).

\subsection{Syntax}
The bytes data type object may be parsed by the EO compiler directly from the source code. The syntax rules for bool values are as follows.
EBNF Notation:
\begin{ffcode}
    BYTES   ::= BYTE ( ‘-’ BYTE ) | ‘--’
    BYTE    ::= [0-9A-F][0-9A-F]
...
The object bytes has these four attributes, to turn
itself into one of four primitive data objects: \texttt{as-int}, \texttt{as-bool}, \texttt{as-float}, and \texttt{as-string}. 
...
The eq attribute object is used for testing if two bytes objects are equal. The eq attribute object has one free attribute x of type bytes that is the second object (the first object is the base object of the eq attribute).
...
Attributes objects and, or, xor are used for the bitwise operation of the same name. This attributes objects has one free attribute x of type bytes that is the second object (the first object is the base object of the and attribute).
...
The not attribute object is used for the bitwise operation of the same name. This attributes objects has no free attributes.
...
Attributes objects left, right are used for left and right bitwise shift. This attributes objects has one free attribute x of type int that is the second object (the first object is the base object of the and attribute).
The free attribute x specifies how many bits to shift.
